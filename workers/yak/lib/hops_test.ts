// The round-trip count, at the end that still exists: the ambient tally
// itself. What is being defended is the reason the count exists. A duration
// says a request got slower; only a count says it got slower because something
// began asking one row at a time, and an N+1 is the bug we keep writing.
//
// The other half of this file measured what a fleet `/query` cost in
// statements, exactly, and went with that server (T-37584). The worker reports
// its own `Server-Timing: hops;dur=<n>` from `counts()` below.
