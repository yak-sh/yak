...
/** One file's text in the shape its reader takes now, or null where it is in
 * that shape already. */
export type Rewrite = (text: string) => string | null

/**
 * An app's files rewritten wherever they are kept — the live bytes, each entry
 * in a path's history, and every deploy manifest — so a rollback or a restore
 * puts the new shape back too. `shapes` names each path and its rewrite. It
 * rides the daily sweep (erase.ts `collected`) the way `renamed` does, and is
 * idempotent for the same reason: a second run finds nothing a rewrite
 * changes.
 *
 * Git history is not rewritten. A commit names the bytes its deploy named then
 * (gitobj.ts), and a clone reads them by that sha, so a manifest's old bytes
 * go under `aside` — git's own key space, which no sweep reaches — before the
 * manifest stops naming them. Only a path's history names its entries' old
 * bytes, so those are left to the sweep.
 *
 * Answers, per path, where it rewrote: `live`, `history`, and each `v<n>`.
 */
export let rewritten = async (
  blobs: Objects,
  dir: Directory,
  { prefix, app }: Pinner,
  shapes: Record<string, Rewrite>,
  aside: string,
) => {
  let said: Record<string, string[]> = {}
  let note = (path: string, where: string) => (said[path] ??= []).push(where)
  let into = recoder(pins(blobs, prefix), shapes)
  for (let [path, to] of Object.entries(shapes)) {
    let live = await blobs.read(prefix + path)
    let text = live && to(new TextDecoder().decode(live))
    if (text) {
      await blobs.put(prefix + path, new TextEncoder().encode(text))
      note(path, 'live')
    }
    let log = await history(blobs, prefix, path)
    let now = await Promise.all(log.map(async (w) => {
      let m = await into(path, w.sha)
      return m ? { ...w, sha: m.sha, size: m.size } : w
    }))
    if (now.some((w, i) => w != log[i])) {
      await blobs.put(
        logKey(prefix, path),
        new TextEncoder().encode(JSON.stringify(now)),
      )
      note(path, 'history')
    }
  }
  ...
  for (let v of await versions(dir, app)) {
    let files = { ...v.files }
    for (let path of Object.keys(shapes)) {
      let sha = files[path]
      let m = sha ? await into(path, sha) : null
      if (!m) continue
      if (!(await blobs.has(aside + sha))) await blobs.put(aside + sha, m.was)
      files[path] = m.sha
      note(path, `v${v.version}`)
    }
    if (!same(files, v.files)) {
      entities.push({
        entity: { eid: v.eid },
...
// A path's pinned bytes through its rewrite, and the rewritten bytes pinned:
// each sha once, since a file unchanged across twenty deploys is one rewrite.
let recoder = (store: Pins, shapes: Record<string, Rewrite>) => {
  let seen = new Map<string, ReturnType<typeof recoded>>()
  let recoded = async (path: string, sha: string) => {
    let was = await store.get(sha)
    let text = was && shapes[path](new TextDecoder().decode(was))
    if (!was || !text) return null
    let bytes = new TextEncoder().encode(text)
    let name = await sha256(bytes)
    await store.put(name, bytes)
    return { was, sha: name, size: bytes.byteLength }
  }
  return (path: string, sha: string) => {
    let key = `${path}\n${sha}`
    if (!seen.has(key)) seen.set(key, recoded(path, sha))
    return seen.get(key)!
  }
}
