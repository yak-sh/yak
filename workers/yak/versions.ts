...
/** A text rewrite of two files into one: the manifest's text (null where the
 * app has none) and the file folded into it, each with the name it was read
 * under, to the manifest's new text — or null where the two cannot be one. */
export type Merge = (
  into: string | null,
  intoFile: string,
  from: string,
...
/**
 * A file folded into another wherever an app keeps them — the live bytes and
 * every deploy manifest — so a rollback or an install puts the folded shape
 * back too (T-38021: a `tools.json` into the `vocab.json` beside it). `from`
 * and `into` list each file's names, the one read first first. It rides the
 * daily sweep (erase.ts `collected`) the way `renamed` does, and is idempotent
 * for the same reason: once `from` is gone, a run reads one name per format
 * and the manifest list.
 *
 * The live file's outgoing bytes go into each path's history (`replaced`), so
 * `app_files` can put either back. Git history is not rewritten: a commit
 * names the bytes its deploy named then (gitobj.ts), so a manifest's old bytes
 * go under `aside` — git's own key space, which no sweep reaches — before the
 * manifest stops naming them.
 *
 * Answers where it folded — `live` and each `v<n>` — and where it could not.
 */
export let folded = async (
  blobs: Objects,
  dir: Directory,
  { prefix, app }: Pinner,
  from: string[],
  ...
  let said = { folded: [] as string[], refused: [] as string[] }
  let text = (b: Uint8Array | null | undefined) =>
    b ? new TextDecoder().decode(b) : null
  let fromFile = await firstOf(blobs, prefix, from)
  if (fromFile) {
    let intoFile = await firstOf(blobs, prefix, into) ?? into.at(-1)!
    let now = merge(
      text(await blobs.read(prefix + intoFile)),
      intoFile,
      text(await blobs.read(prefix + fromFile))!,
      fromFile,
    )
    if (now == null) said.refused.push('live')
    else {
      await replaced(blobs, prefix, intoFile, '')
      await replaced(blobs, prefix, fromFile, '')
      await blobs.put(prefix + intoFile, new TextEncoder().encode(now))
      await blobs.delete(prefix + fromFile)
      said.folded.push('live')
    }
  }
  let store = pins(blobs, prefix)
  let entities = []
  for (let v of await versions(dir, app)) {
    let f = from.find((p) => p in v.files)
    if (!f) continue
    let i = into.find((p) => p in v.files)
    let was = await store.get(v.files[f])
    let had = i ? await store.get(v.files[i]) : null
    let now = was && (!i || had)
      ? merge(text(had), i ?? into.at(-1)!, text(was)!, f)
      : null
    if (now == null) {
      said.refused.push(`v${v.version}`)
      continue
    }
    let bytes = new TextEncoder().encode(now)
    let sha = await sha256(bytes)
    await store.put(sha, bytes)
    for (let [path, b] of [[f, was], [i, had]] as const) {
      if (path && b && !(await blobs.has(aside + v.files[path]))) {
        await blobs.put(aside + v.files[path], b)
      }
    }
    let { [f]: _, ...files } = v.files
    files[i ?? into.at(-1)!] = sha
    entities.push({
      entity: { eid: v.eid },
      deploy: { files: JSON.stringify(files) },
    })
    said.folded.push(`v${v.version}`)
  }
  if (entities.length) await dir.apply({ entities })
  return said
}

// The first of a file's names the app holds, or null.
let firstOf = async (blobs: Objects, prefix: string, names: string[]) => {
  for (let name of names) if (await blobs.has(prefix + name)) return name
  return null
}
