...
export let RuntimePanel = ({ ui, agent, session, subscribe }: {
  ui: Frontend; agent: UIAgent; session?: string; subscribe: (fn: () => void) => () => void
}) => {
  ...
  useLayoutEffect(() => {
    ...
    let load = async () => {
      ...
      try {
        ...
        busy = false
        if (dirty && alive) { dirty = false; void load() }
      }
    }
    void load()
  ...
  useKeys((key) => {
    ...
    if (key.name == 'escape') { ui.keys({ runtime: false }); return true }
    let text = key.text
    let list = rows.value
    let index = Math.max(0, list.findIndex((b) => b.entity.eid == current.runtimeSelected))
    let selected = list[index]
    if (text == 'j' || text == 'k') {
      let next = list[Math.max(0, Math.min(list.length - 1, index + (text == 'j' ? 1 : -1)))]
      if (next) ui.keys({ runtimeSelected: next.entity.eid })
    } else if ((text == 'x' || text == 'c') && selected && agent.control) {
      let action = text == 'c' ? 'resume' : (selected.dispatch as Comp | undefined)?.state == 'queued' ? 'cancel-queued' : 'interrupt'
  ...
  if (!open) return null
  let index = Math.max(0, rows.value.findIndex((b) => b.entity.eid == state.runtimeSelected))
  return h('div', { border: 'Composer_Border', height: '10', col: '1' },
    h('div', { class: 'Panel_Title' }, 'Runtime · j/k select · x interrupt/cancel queued · c continue · r close'),
    h(Scroll, { id: 'runtime', grow: '1', keyboard: false, reveal: index },
      ...rows.value.map((b, i) => h('div', { key: b.entity.eid, fill: '1', class: i == index ? 'Session_Selected' : '' },
        render(runtimeViews, b, 'Runtime', vocabulary), ' ',
        elapsed((b.updated as Comp | undefined)?.at ?? (b.created as Comp | undefined)?.at, now.value), ' · ', sessionLine(b))),
    ),
    !session ? h('div', null, 'Select a session first.') : null,
    h('div', { wrap: '1', class: 'Muted' }, String(state.runtimeFeedback ?? 'Actions affect only the selected session, never independent processes or task completion.')),
  )
}
