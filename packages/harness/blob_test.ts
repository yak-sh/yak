...
let bodyOf = (b: Bundle, comp = 'content') => (b[comp] as Comp | undefined)?.body
...
Deno.test('blob prose deduplicates across properties, survives reopen and migrates legacy text exactly', async () => {
  ...
  try {
    ...
    assertEquals(h.db.prepare('select count(*) as n from blob_text').get(), { n: 1 })
    assertEquals(h.db.prepare('select body from content').get(), { body: address(body) })
    ...
    h.db.exec("delete from harness_upgrade; update content set body = '" + address(body) + "'; update doc set body = 'legacy'")
    h.close()
    h = open(path)
    ...
    h = open(path)
    assertEquals(bodyOf((await h.g.read('.content'))[0]), address(body))
    await h.g.apply([{ entity: { eid: 'a' }, content: { body: 'updated' }, $was: { content: { body: address(address(body)) } } }])
