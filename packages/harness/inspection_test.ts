...
Deno.test('inspection searches a fork prefix in bounded pages and reads exact Unicode source', async () => {
  ...
  try {
    await h.g.apply([
      ...
      { entity: { eid: 'a' }, entry: { session: 'p' }, content: { body: '🌲first needle\nneedle' } },
      { entity: { eid: 'outside' }, entry: { session: 'p' }, content: { body: 'needle outside fork' } },
      { entity: { eid: 'c' }, session: { id: 'child' }, fork: { from: 'a' } },
      { entity: { eid: 'own' }, entry: { session: 'c' }, content: { body: 'needle '.repeat(45) } },
    ])
    ...
    await assertRejects(() => api.inspect('c', 'a', 0, page.revision), Error, 'revision changed')
...
Deno.test('search bounds no-match scanning and detail responses', async () => {
  const h = open(':memory:')
  try {
    await h.g.apply([{ entity: { eid: 'p' }, session: { id: 'p' } },
      ...Array.from({ length: 80 }, (_, i) => ({ entity: { eid: 'e' + i }, entry: { session: 'p' }, content: { body: i == 79 ? 'found' : 'absent' } })),
      { entity: { eid: 'huge' }, entry: { session: 'p' }, prompt: { scope: 'shared' }, content: { body: 'x'.repeat(100000) } },
    ])
    const api = inspection(h.g)
    const page = await api.search('p', 'found')
    assertEquals(page.matches, [])
    assert(page.next)
    assertEquals((await api.search('p', 'found', page.next)).matches[0].entity, 'e79')
