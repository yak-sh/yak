...
export const inspection = (g: Graph): Inspection => {
  const row = async (id: string) =>
    (await g.storage.tx((tx) => tx.get([id])))[0]
  ...
  return {
    async inspect(session, entity, start = 0, revision) {
      ...
      if (
        !e ||
        !plan.some((p) => p.session == e.session && Number(e.seq) <= p.max)
      ) {
    ...
    async search(session, query, cursor = { segment: 0, seq: 0, offset: 0 }) {
      if (!query || query.length > 256) {
        throw new Error('Search needs 1–256 characters')
      }
      const plan = await segments(session)
      if (
        ![cursor.segment, cursor.offset].every((n) =>
          Number.isInteger(n) && n >= 0
        ) || !Number.isFinite(cursor.seq) || cursor.seq < 0
      ...
      for (
        let scanned = 0;
        scanned < 64 && at.segment < plan.length;
        scanned++
      ) {
        const p = plan[at.segment]
        const [b] = await g.read(
          '.entry.session=' + p.session + '&.content&.entry.seq' +
            (at.offset ? '>=' : '>') + at.seq +
            (Number.isFinite(p.max) ? '&.entry.seq<=' + p.max : '') +
            '&.order=entry.seq&.limit=1',
        )
        if (!b) {
          at = { segment: at.segment + 1, seq: 0, offset: 0 }
          continue
        }
        ...
        const result = JSON.parse(
          await search.run({
            ...
            query,
            start: at.offset,
            limit: 20 - matches.length,
          }),
        ) as {
          revision: string
          matches: { offset: number; text: string }[]
          next: number | null
        }
        matches.push(
