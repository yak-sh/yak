...
import { type Key, pressTo, Scroll } from '@yaks/tui'
...
export const inspector = (
  ...
  const find = async (fresh: boolean) => {
    ...
    const cursor = fresh
      ? undefined
      : JSON.parse(String(state().cursor || 'null')) as SearchCursor | null
    const was = fresh
      ? []
      : JSON.parse(String(state().matches)) as SourceMatch[]
    ...
    try {
      ...
      set({
        ...
        cursor: page.next ? JSON.stringify(page.next) : null,
        index: matches.length
          ? (fresh ? 0 : Math.min(was.length, matches.length - 1))
          : -1,
        message: matches.length
          ? matches.length + ' matches loaded'
          : page.next
          ? 'No matches in this page; n searches further'
          : 'No matches',
      })
  ...
  return (key: Key): boolean => {
    ...
    if ((ui.client.ent('keyboard')!.keyboard as Comp).help) return false
    if ((ui.client.ent('keyboard')!.keyboard as Comp).mode != 'NORMAL') {
      return false
    }
    ...
    if (s.mode == 'query') {
      if (key.name == 'escape') {
        set({ mode: '', message: '', generation: Number(s.generation) + 1 })
      } else if (key.name == 'enter') {
        if (s.query) void find(true)
        else set({ mode: '' })
      } else if (key.name == 'backspace') {
        set({ query: Array.from(String(s.query)).slice(0, -1).join('') })
      } else if (text || key.name == 'paste') {
        set({
          query: (String(s.query) + (key.text ?? '')).replace(/[\r\n]/g, ' ')
            .slice(0, 256),
        })
      }
      return true
    ...
    if (s.mode == 'detail') {
      if (key.name == 'escape' || text == 'q') {
        set({ mode: '', message: '', generation: Number(s.generation) + 1 })
      } else if (text == ']' && s.next != null) {
        void inspect(String(s.entity), Number(s.next), String(s.revision))
      } else if (
        text == 'j' || text == 'k' || key.name == 'down' || key.name == 'up' ||
        key.name == 'pageup' || key.name == 'pagedown'
      ) {
        pressTo('entry-detail', {
          name: text == 'j' ? 'down' : text == 'k' ? 'up' : key.name,
        })
      } else if (text == '[' && Number(s.start) > 0) {
        void inspect(
          String(s.entity),
          Math.max(0, Number(s.start) - 4096),
          String(s.revision),
        )
      }
      return true
    ...
    if ((text == 'n' || text == 'N') && s.query) {
      ...
      if (text == 'n' && index >= matches.length && s.cursor) {
        void find(false)
        return true
      }
      if (matches.length) {
    ...
    if (
      key.name == 'enter' && session() &&
      (ui.client.ent('keyboard')!.keyboard as Comp).focus == 'transcript'
    ) {
      const viewport = ui.client.ent('viewport-' + session())?.viewport as
        | Comp
        | undefined
      const entity = String(
        viewport?.selected ?? viewport?.item ?? s.entity ?? '',
      )
      if (entity) {
        const matches = JSON.parse(String(s.matches)) as SourceMatch[]
        const match = matches[Number(s.index)]
        void inspect(
          entity,
          0,
          match?.entity == entity ? match.revision : undefined,
        )
      }
      return true
    }
...
const detailVocab = loadVocab([{
  $defs: Object.fromEntries(
    ['prompt', 'result', 'error', 'exception', 'attachment', 'ask'].map((
      name,
    ) => [name, { properties: {} }]),
  ),
}])
const detail = (match: string | true, label: string): Renderer => ({
  ...
  render: (_b, host, ctx) =>
    host(
      'div',
      null,
      host('div', { class: 'Entry_Hint' }, label + ' · literal source'),
      host('div', { wrap: '1' }, String(ctx.text ?? '')),
    ),
})
export const detailViews = define([
  detail('.prompt', 'Instructions'),
  detail('.result', 'Tool result'),
...
export const InspectionPanel = ({ ui }: { ui: Frontend }) => {
  ...
  const currentSession = String(
    (ui.view.value[0].frontend as Comp).selected ?? '',
  )
  if (s.session != currentSession || (!s.mode && !s.message)) return null
  return h(
    'div',
    {
      border: 'Composer_Border',
      ...(s.mode == 'detail' ? { height: '12', col: '1' } : {}),
    },
    s.mode == 'query'
      ? '/' + String(s.query) + ' · Enter search · Esc cancel'
      : s.mode == 'detail'
      ? h(
        'div',
        { col: '1', grow: '1' },
        h(
          ...
          { class: 'Entry_Hint' },
          'Source ' + s.start + '–' +
            (Number(s.start) + Array.from(String(s.text ?? '')).length) +
            ' / ' + s.total + ' · [/] chunks · Esc close',
        ),
        h(
          Scroll,
          {
            key: String(s.entity) + ':' + s.start,
            id: 'entry-detail',
            grow: '1',
            follow: false,
          },
          render(
            detailViews,
            JSON.parse(
              String(s.metadata || '{"entity":{"eid":"detail"}}'),
            ) as Bundle,
            'Detail',
            detailVocab,
            { text: s.text },
      ...
      : null,
    s.message
      ? h('div', { class: 'Entry_Hint', wrap: '1' }, String(s.message))
      : null,
  )
}
