...
export let runtimeRows = async (
  ...
  return await Promise.all(
    rows.filter((b) =>
      b.entity.eid == session ||
      (b.spawned as Comp | undefined)?.parent == session
    ).map(async (b) => {
      let id = b.entity.eid
...
export let runtimeAction = async (
  ...
  if ((row.session as Comp).status != 'settled') {
    throw new Error(
      'Resume requires a settled session; running/stopped work is not replayed',
    )
  }
  await a.send(
    session,
