...
Deno.test('tree expands lazily, retains selected ancestry and hides archived subtrees', () => {
  let rows = [row('a'), row('b'), row('c', 'a'), row('d', 'c', 'settled')]
  let ids = (o = {}) =>
    sessionTree(rows, o).map((r) => [r.bundle.entity.eid, r.depth])
  assertEquals(ids(), [['a', 0], ['b', 0]])
  assertEquals(ids({ expanded: ['a', 'c'] }), [['a', 0], ['c', 1], ['b', 0]])
  assertEquals(ids({ selected: 'd' }), [['a', 0], ['c', 1], ['d', 2], ['b', 0]])
  assertEquals(rootOf(rows, 'd'), 'a')
  rows[0].archived = {}
  assertEquals(ids({ expanded: ['a'] }), [['b', 0]])
  assertEquals(ids({ showArchived: true }), [['a', 0], ['b', 0]])
})
Deno.test('orphan roots and corrupt cycles never loop', () => {
  ...
  assertEquals(sessionTree(cycle, { expanded: ['x', 'y'] }).length, 2)
})
