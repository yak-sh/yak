...
export let Textarea = (
  {
    ...
    max?: number
    prompt?: string
    /** Return false to retain a rejected draft; other return values clear it. */
  ...
  useKeys((k) => {
    ...
    if (k.name == 'enter' && !k.shift && !k.alt) {
      const accepted = now.text ? onSubmit?.(now.text) : undefined
      if (accepted !== false) take({ text: '', at: 0 })
      return true
    }
