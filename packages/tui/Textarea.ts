...
export let Textarea = (
  { id = 'input', max = 8, prompt = '> ', onSubmit, onChange }: {
    id?: string
    max?: number
    prompt?: string
  ...
  let [s, set] = useState<Edit>({ text: '', at: 0 })
  ...
  let { row, col } = spot(s)
  let height = Math.min(Math.max(1, lines.length), max)
  let top = Math.min(
    Math.max(0, row - height + 1),
    Math.max(0, lines.length - height),
  )
  let gutter = (i: number) =>
    h('span', { class: 'Entry_Hint' }, i ? ' '.repeat(prompt.length) : prompt)
  return h(
    'div',
