...
import type { AuthorizationStore } from './mod.ts'

export const fileAuthorizationStore = <R extends object>(
  path: string,
  validate: (record: R) => void = () => {},
): AuthorizationStore<R> => {
  const dir = path.slice(0, path.lastIndexOf('/')) || '.'
  const load = async (): Promise<
    { version: 1; records: Record<string, R> }
  > => {
    try {
  ...
  return {
    ...
    update: async (key, fn) => {
      ...
      try {
        ...
        const record = data.records[key] ??= {} as R
        let result
        try {
