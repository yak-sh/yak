...
// A context over a fixed set of bundles: `read` answers the citations among
// them, and `get` answers by id, which is all these two tools ask for.
let ctx = (
...
Deno.test('a citation of an entity with no journal to read says so rather than passing', async () => {
  ...
  let said = body(await tools.cites_check!([], ctx({}, bundles)))
...
Deno.test('check is scoped by the entity citing, and by the file cited', async () => {
  ...
  let mine = body(await tools.cites_check!([], ctx({ of: 'doc-1' }, bundles)))
  assert(mine.includes('src/db.ts'), mine)
  assert(!mine.includes('src/cli.ts'), mine)
  let one = body(
    await tools.cites_check!([], ctx({ path: 'src/cli.ts' }, bundles)),
  )
...
Deno.test('a check with nothing to report still answers, and reports no fault', async () => {
  let answer = await tools.cites_check!([], ctx({}, [doc('doc-1', 1)]))
...
Deno.test('verify refuses an id that cites nothing, before it asks git anything', async () => {
  ...
  await assertRejects(
    () =>
      Promise.resolve(tools.cites_verify!([], ctx({ cite: 'doc-1' }, bundles))),
    Error,
    'not a citation',
  )
  await assertRejects(
    () => Promise.resolve(tools.cites_verify!([], ctx({}, bundles))),
    Error,
...
Deno.test('neither tool runs where the graph stands in no checkout', async () => {
  let at = ctx({}, [])
  await assertRejects(
    () => Promise.resolve(tools.cites_check!([], { ...at, cwd: undefined })),
    Error,
...
slow(
  ...
  async () => {
    ...
    let answer = await tools.cites_check!([], ctx({}, bundles, cwd))
...
slow(
  ...
  async () => {
    ...
    let [written] = await tools.cites_verify!(
      [],
      ctx({ cite: 'cite-doc-1-file-1' }, bundles, cwd),
    )
    ...
    let all = await tools.cites_verify!([], ctx({ of: 'doc-1' }, bundles, cwd))
