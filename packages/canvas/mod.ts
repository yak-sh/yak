 ...
 * `card` showing something, and its `pin` records where on the wall it sits.
 ...
 *
 * All of it is entities carrying components, which is the whole idea: a layout
 ...
 *
 * ## One rule, declared
 * A card is deleted with the thing it shows — `death: cascade` on
 * `card.target`, which the graph enforces in the same transaction as the delete
 * that caused it. There is no hook and no orphan sweep.
 *
 ...
 * {@link visible} is what a renderer calls every frame, {@link place} computes
 * where the next card goes, and {@link frame} computes the camera that fits
 * everything. They take plain objects and touch no platform API, so a browser
 * canvas and a terminal compute the same result rather than drifting apart.
 *
 * @module
 */
