...
//
// This is the handler slot @yaks/member documents and leaves empty. Adding a
// person to a roster usually means writing to them, but a roster package must
...
/** The roster row that was just written, as the letter's author sees it. */
export type Seat = {
...
/** The letter to send about a new roster row, or `null` to send none. */
...
(event) => {
  ...
  return apply([{
    entity: { eid: eid() },
    // The words, in the component every readable thing has (@yaks/doc).
