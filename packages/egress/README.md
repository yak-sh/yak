...
`forward(ctx, {app, level}, request)` returns the service's response. `ctx` is
@yaks/connections' `Ctx`: the graph, its vault, the built integrations and the
OAuth client registered with each. `level` is what the host vouches the caller
holds on the app ([@yaks/member](../member)), or `null` for nothing.
...
- The caller must be allowed to call out through it: @yaks/member's
  ...
  `anyone`, as a public widget's key is. A sentinel points at a credential and
  is not one, so one that leaks into a page is useless to a stranger.
