...
| `lease`    | Job lease duration in milliseconds; defaults to `30000`.                                         |
| `jobs`     | Whether this process runs the jobs; defaults to `true`.                                          |
...
handler, tool runner, jobs, process entity, request authentication function, and
shutdown signal. An entry without `with` receives an empty object. Option keys
...
| `./service` | `service?: (host, options, signal)` for leased jobs                                           |
...
`host.runner`, and `host.jobs`, but must not access them before initialization.
In particular, `rules.extend` runs before the store exists. A route is an
`@yaks/api` `Route` with `method`, `path`, and a `(Request) => Response`
...
`routes`, `handler`, `runner`, and `jobs` — plus `fx` and `close`. Nothing here
binds a port. The `serve` tool does that, reading the handler off the host it
was composed into, reconciling interrupted calls, and taking over the jobs for
as long as it listens.

## Jobs: the work nobody is asking for

The effect sweep and each plugin's `./service` export are jobs (`host.jobs`).
Each runs under a lease named for its owning package, so only one process over a
graph runs it at a time.

```ts
await host.jobs() // Run until the host shuts down.
await host.jobs(AbortSignal.abort()) // Run one pass, then release leases.
```
...
`yak --no-jobs` (config `jobs: false`) turns them off for one process: it takes
no lease, and runs neither the sweep, the services, nor the start-up passes
`@yaks/session` and `@yaks/spawn` hold a lease for. A one-shot command then only
runs its tool, and `yak --no-jobs serve` answers requests while another process,
or none, does the jobs.
...
| `--no-jobs`       | Take no lease and run no jobs (config `jobs: false`).                  |
