...
// A plugin that says something at the start of a transcript, and where it
// sits. What it says names the transcript, so the test can see each
// contributor was asked about the same one.
let saying = (heading: string, weight: number): Plugged => ({
  digest: {
    weight,
    digest: () => (session) => [{
      heading,
      lines: [`${heading}: ${session.entity.eid}`],
    }],
  },
})

Deno.test("the digest is every plugin's sections, in weight order", async () => {
  let mods = {
    shop,
    late: saying('goals', 20),
    early: saying('owner said', -10),
  }
  // Named late-first on purpose: the config's order is not the digest's.
  let host = await compose({
    db: ':memory:',
    plugins: ['late', 'early', 'shop'],
  }, only(mods))
  try {
    let told = await host.digest({ entity: { eid: 's1' } }, host.graph.read)
    assertEquals(told, [
      { heading: 'owner said', lines: ['owner said: s1'] },
      { heading: 'goals', lines: ['goals: s1'] },
    ])
  } finally {
    host.close()
  }
...
Deno.test('a plugin nobody composed contributes no section, and no error', async () => {
  let host = await compose({
    db: ':memory:',
    plugins: ['early', 'shop'],
  }, only({ shop, early: saying('owner said', -10) }))
  try {
    let told = await host.digest({ entity: { eid: 's1' } }, host.graph.read)
    assertEquals(told.map((s) => s.heading), ['owner said'])
  } finally {
    host.close()
  }
})
