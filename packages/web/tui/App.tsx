...
let file: Ctx['read'] = (p) => {
  ...
  try {
    ...
    throw new Error(`.${p.prop}=${v}: no such file`)
  }
}
...
export let navigationKey = (k: string) => {
  ...
  let size = Math.max(1, favorites.length)
...
export let TNavigation = () => {
  ...
  let pick = Math.min(navigationPick.value, favorites.length - 1)
  return (
