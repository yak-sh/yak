...
let responses = (options: ResponseOptions) =>
  transport({ ...options, redact: true })
...
Deno.test('responses refreshes once on 401 and never returns credentials', async () => {
  ...
  assertEquals(
    observed[0].delta,
    '[redacted] [redacted] [redacted] safe',
  )
})
...
Deno.test('redaction is opt-in and remembers credentials across runs', async () => {
  for (let redact of [false, true]) {
    ...
    let client = transport({
      redact,
      credentials: {
        get: () => ({ token: ++turn == 1 ? 'old-key' : 'new-key' }),
      },
      fetch: () =>
        Promise.resolve(sse(
          { type: 'response.output_text.delta', delta: 'old-key new-key' },
          {
            type: 'response.completed',
            response: { status: 'completed', model: 'm' },
          },
    ...
    assertEquals(
      observed[0].delta,
      redact ? '[redacted] [redacted]' : 'old-key new-key',
    )
  }
})
