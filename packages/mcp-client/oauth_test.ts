...
import { authorization, AuthorizationError } from './oauth.ts'
import { fileAuthorizationStore } from './host.ts'
...
Deno.test('paste OAuth uses SDK discovery/DCR/PKCE, validates callbacks and persists rotated tokens', async () => {
  ...
  try {
    ...
    const contents = await Deno.readTextFile(dir + '/auth.json')
    assert(!contents.includes('code-secret'))
    assert(!contents.includes(url.searchParams.get('code_challenge')!))
    assertEquals((await Deno.stat(dir + '/auth.json')).mode! & 0o777, 0o600)
...
Deno.test('private file updates serialize concurrent writers and reject permissive files', async () => {
  const dir = await Deno.makeTempDir()
  try {
    const file = dir + '/auth.json'
    const first = fileAuthorizationStore(file),
      second = fileAuthorizationStore(file)
    await Promise.all([
      first.update('a', async (r) => {
        r.tokens = { access_token: 'a', token_type: 'Bearer' }
        await Promise.resolve()
      }),
      second.update('b', async (r) => {
        r.tokens = { access_token: 'b', token_type: 'Bearer' }
        await Promise.resolve()
      }),
    ])
    assertEquals((await first.read('a'))?.tokens?.access_token, 'a')
    assertEquals((await first.read('b'))?.tokens?.access_token, 'b')
    await Deno.chmod(file, 0o644)
    await assertRejects(() => first.read('a'), Error, 'private permissions')
  } finally {
    await Deno.remove(dir, { recursive: true })
  }
})
