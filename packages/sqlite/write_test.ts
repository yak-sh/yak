...
// a `$delete` entity tombstoned — and the death each reference word spreads.
...
Deno.test('a patch touches only the columns it names', () => {
  let s = store()
  s.write([{ entity: { eid: 'p1' }, product: { price: 12, status: 'live' } }])
  s.write([{ entity: { eid: 'p1' }, product: { price: 15 } }])
...
Deno.test('a null column clears it, its siblings untouched', () => {
  let s = store()
  s.write([{ entity: { eid: 'p1' }, product: { price: 12, status: 'live' } }])
  s.write([{ entity: { eid: 'p1' }, product: { status: null } }])
  let [p] = s.read('.kind=product')
...
Deno.test('a null component drops the row, the entity survives', () => {
  ...
  s.write([{ entity: { eid: 'p1' }, product: null }])
...
Deno.test('a boolean round-trips through integer storage', () => {
  let s = store()
  s.write([{ entity: { eid: 'p1' }, product: { available: true } }])
  assertEquals(c(s.read('.kind=product')[0], 'product').available, 1)
})

Deno.test('$delete tombstones an entity and it leaves every read', () => {
  let s = store()
  s.write([{ entity: { eid: 'p1' }, doc: { title: 'Mug' } }])
  let dead = s.write([{ entity: { eid: 'p1' }, $delete: true }])
...
Deno.test('a cascade reference pulls its owner into the grave', () => {
  ...
  let dead = s.write([{ entity: { eid: 'p1' }, $delete: true }]).sort()
...
Deno.test('a detach reference is nulled when its target dies', () => {
  ...
  let dead = s.write([{ entity: { eid: 'm1' }, $delete: true }])
...
Deno.test('a release reference drops its row, its owner lives', () => {
  ...
  let dead = s.write([{ entity: { eid: 'd1' }, $delete: true }])
...
Deno.test('a write with no deletion returns no casualties', () => {
  let s = store()
  assertEquals(s.write([{ entity: { eid: 'p1' }, product: { price: 1 } }]), [])
})
