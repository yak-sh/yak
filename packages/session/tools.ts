...
//
// WHAT A SESSION IS TOLD IS NOT THIS PACKAGE'S ALONE. `session_context`
// answers the digest the HOST composed — every plugin's sections, in weight
// order (@yaks/context `composed`, ./digest.ts for this package's own). Taken
// alone, with no host to compose anything, it still answers this package's
// three; the fallback is the same factory, composed by itself.
//
...
import { composed, type Digest, written } from '@yaks/context'
...
import { digest, line, weight } from './digest.ts'
 ...
 * stall by. */
export let runs = (
  host: { vocab: Vocab; digest?: Digest },
  options: Options = {},
): Runs => {
  // Where the host composed a digest, that is the one — every plugin's
  // sections, in weight order. Where nothing did, this package's tools taken
  // alone still answer its own three, composed by themselves.
  let told = host.digest ?? composed([{ sections: digest(host), weight }])
  return {
        ...
        { ...s, [SESSION]: { id, ...(actor ? { actor } : {}) } },
        {
          ...
          content: {
            body: [
              ...
              ...written(await told(s, (q, o) => ctx.read(q, o))),
            ].join('\n'),
          },
        },
      ]
