...
Deno.test('append starts after a nested fork boundary', async () => {
  ...
  seed(
    ...
    { entity: { eid: 'b' }, entry: { session: 'parent', seq: 2 } },
    { entity: { eid: 'c' }, entry: { session: 'parent', seq: 3 } },
    { entity: { eid: 'child' }, session: {}, fork: { from: 'b' } },
    { entity: { eid: 'd' }, entry: { session: 'child', seq: 3 } },
    { entity: { eid: 'grand' }, session: {}, fork: { from: 'd' } },
    { entity: { eid: 'e' }, entry: { session: 'grand', seq: 4 } },
  )
  let g = locked(s)
  assertEquals(
    (await transcript(g, 'grand')).map((b) => b.entity.eid),
    ['a', 'b', 'd', 'e'],
  )
