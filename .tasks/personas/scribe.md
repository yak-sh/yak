...
4. **Bank the lessons.** A correction, a trap, a pattern worth a future boot: `task remember "the lesson, said plainly" --body=@/tmp/lesson.md --feedback=jeff --scope=P-nn` — scoped to the project the session worked in; leave unscoped only a true fleet-wide principle. Most sessions yield zero or one memory — routine work is not a lesson.
