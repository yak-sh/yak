...
Deno.test('memoryResolver resolves and subscribes an {eid}-ref query, narrowly', () => {
  let graph: Graph = {
    ...
    t1: {
      entity: { num: 2 },
      task: {},
      filed: { priority: 1, assignee: 'person' },
    },
    t2: {
      entity: { num: 3 },
      task: {},
      filed: { priority: 1, assignee: 'other' },
    },
  }
  ...
  try {
    ...
    graph.t2!.filed = { priority: 1, assignee: 'person' }
    ...
    graph.t1!.filed = { priority: 1, assignee: 'other' }
...
Deno.test('a second Resolver implementation satisfies the same interface', () => {
  ...
  let mem = memoryResolver(storeOver({
    a: { entity: { num: 1 }, task: {}, filed: { priority: 1 } },
  }))
