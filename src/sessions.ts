...
// The managed stdout's HISTORY half, standalone (D-16704): one source line → its
// entry batch, source 'managed', skip-if-present — the SAME mapping drain() does
// inline beside its summary stamp, factored out so the historical backfill
// (T-16822) reuses it over a FINISHED run without re-stamping that run's settled
// liveness columns. The `session.prompt`/`session.input` markers are ours — the
// user's turn — a resume (`session.input`) reopens the log past a prior run's
// terminal, and provider noise after the terminal is diagnosed, never a row.
// t.seq is the source coordinate and advances over EVERY line, blank or
// malformed alike, so it stays stable across a re-read; only recognized lines
// become entries. Mirrors ingestNative; the mapper (ingestEntries) is all that
// differs from drain()'s per-line history step.
let ingestManaged = (
  eid: string,
  ad: Adapter,
  t: Tail,
  ...
  for (let line of lines) {
    ...
    let type = (e as { type?: unknown }).type
    if (type == 'session.prompt' || type == 'session.input') {
      ingestLine(eid, state, t.seq, {
        specs: [{
          message: { role: 'user' },
          content: { body: scrub((e as { text?: unknown }).text) },
        }],
        ids: [uuid()],
        calls: [],
      }, t.errs)
      if (type == 'session.input') t.ended = false // a resume reopens the log
      continue
    }
    if (t.ended) {
      if (!t.errs.some((x) => x.includes('after the terminal'))) {
    ...
    if (ad.terminal(e)) t.ended = true // the last word — gate the tail off
    ingestLine(
      eid,
      state,
      t.seq,
      ingestEntries(ad.dialect, e, state.state),
      t.errs,
    )
  }
}
...
// Does this session already carry a durable failure facet? Backfill fills an
// EMPTY error slot but never overwrites a settled run's own reason (M-16612:
// preserve each error under its stable identity) — a failed launch already
// explains itself, and its source-gap note would be the lesser truth.
let hasError = (eid: string) =>
  !!db.prepare('select 1 from error where eid = ?').get(eid)

let statSize = (path: string): number | null => {
  try {
...
// A completed/interrupted managed run WROTE a log (the file IS its log); if that
// file is gone, its history is unrecoverable — a genuine gap worth a durable
// note. A launch that FAILED before starting legitimately has none, and its
// failure is its own story (see hasError), so a missing log there is silence,
// not a fault.
let ranManaged = (s: Session) =>
  s.status == 'completed' || s.status == 'interrupted' || !!s.started_at

// The historical backfill (T-16822): reconcile every FINISHED file-first Session
// whose source predates live ingestion into the same ordered entry partition,
// with the SAME importers and the SAME derived-coordinate dedup as the live
// tailer — HISTORY only, never a summary re-stamp of a settled run. It is the
// boot-time catch-up for exactly the substrates recover() bounds AWAY: managed
// runs that already ended (recover only re-drains the active set) and native
// transcripts already finished (recover only ingests finished_at IS NULL). The
// two never overlap, so no partition ever has two writers.
//
// Additive and rerun-safe by construction: skip-if-present on
// (session, source, line) means a second pass appends nothing (assert the eid
// set is unchanged). The default pass is INCREMENTAL — it skips a session that
// already holds imported history for its source, so a post-migration boot costs
// one indexed query per remaining session and nothing for the rest. `full`
// drops that filter and re-reads every targeted session: the belt-and-suspenders
// rerun that proves zero duplicates and heals a crash-partial file line by line.
//
// Diagnostics, never silent skips (M-16612): a gone/unreadable source, a
// malformed or oversized line, becomes the session's durable, retryable `error`
// facet (dedup-stable across re-runs), leaving every already-ingested entry
// intact. Single-writer is held by SCOPE (finished / inactive sessions have no
// live tailer) and by skipping anything `running` re-adopted; a session that
// races back to life between the query and its turn is stepped over.
export let backfill = async (cast: Cast, full = false) => {
  let t0 = Date.now()
  let active = sessionActive.map((s) => `'${s}'`).join(', ')
  let before = entryTotal()
  let seen = 0, faults = 0

  let commit = (eid: string, t: Tail) => {
    if (t.errs.length && !hasError(eid)) {
      stamp(eid, { error: diagnosis(t) }, cast)
      faults++
    }
  }
  let step = (phase: string, i: number, total: number) => {
    if (i % 50 == 0 || i == total) {
      console.log(`backfill: ${phase} ${i}/${total} (${faults} diagnosed)`)
    }
  }
  ...
  // recover()/following()'s, never ours. NULL status is inactive (a run that
  // never reached 'starting'), so it must be admitted explicitly.
  let mPrior = full ? '' : `and not exists (
      select 1 from entry e join imported i on i.eid = e.eid
      where e.session = session.eid and i.source = '${MANAGED}')`
  let managed = storedSessions(
    db,
    `where origin = 'managed'
  ...
  for (let [i, s] of managed.entries()) {
    ...
    seen++
    let t: Tail = { at: 0, seq: 0, ended: false, errs: [] }
    let ad = adapters[String(s.provider)]
  ...
  // NATIVE — finished provider transcripts. recover() owns finished_at IS NULL;
  // this owns the rest. transcriptOf() re-derives the confined path, so a
  // transcript the provider has since deleted or moved out of its store resolves
  // to nothing and becomes a durable gap note rather than a silent skip.
  let nPrior = full ? '' : `and not exists (
      select 1 from entry e join imported i on i.eid = e.eid
      where e.session = session.eid and i.source = '${NATIVE}')`
  let native = storedSessions(
    db,
    `where origin != 'managed' and transcript is not null
       and finished_at is not null ${nPrior}`,
  )
  ...
  for (let [i, s] of native.entries()) {
    ...
    commit(s.eid, t)
    step('native', i + 1, native.length)
  }

  let added = entryTotal() - before
  console.log(
    `backfill: swept ${seen} sessions, ${added} entries ingested, ` +
      `${faults} diagnosed, ${Date.now() - t0}ms`,
  )
  return { seen, added, faults }
}

let entryTotal = () =>
  (db.prepare('select count(*) as n from entry').get() as { n: number }).n
