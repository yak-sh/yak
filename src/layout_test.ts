...
let row = [
  p('r', undefined, { dir: 'h' }),
  p('a', 'r', { size: 1, order: 0, content_eid: 'T1', view: 'Full' }),
  p('b', 'r', { size: 1, order: 1, content_eid: 'T2' }),
...
Deno.test('split along the dir is one empty sibling, ordered between', () => {
  ...
  assertEquals(born.parent_eid, 'r')
  assertEquals(born.size, 1)
  assertEquals(born.order, 0.5) // between a (0) and b (1) — nothing renumbers
  assertEquals(born.content_eid, undefined) // empty: it renders the palette
})
...
Deno.test('split across nests: the pane becomes the container', () => {
  ...
  assertEquals(turn, { dir: 'v', content_eid: null, view: null })
  assertEquals(moved.parent_eid, 'a')
  assertEquals(moved.content_eid, 'T1') // the content moved down, view along
  assertEquals(moved.view, 'Full')
  assertEquals(empty.parent_eid, 'a')
...
Deno.test('split a container across hoists its kids under an intermediate', () => {
  ...
  let reparented = batch.filter((c) =>
    ['a', 'b', 'c'].includes(c.eid) && c.comp?.parent_eid
  )
  assertEquals(reparented.length, 3)
  let mid = batch.find((c) => c.comp?.dir == 'h')!
  assertEquals(mid.comp!.parent_eid, 'r') // the old h-row, one level down
  assertEquals(reparented.every((c) => c.comp!.parent_eid == mid.eid), true)
})
...
Deno.test('close collapsing to a leaf survivor hoists content into the parent', () => {
  let pair = [
    p('r', undefined, { dir: 'h' }),
    p('a', 'r', { order: 0, content_eid: 'T1', view: 'Full' }),
    p('b', 'r', { order: 1, content_eid: 'T2', view: 'Board' }),
  ]
  let batch = close(pair, 'a')
  assertEquals(deads(batch), ['a', 'b']) // both children go
  assertEquals(comps(batch), [
    { dir: null, content_eid: 'T2', view: 'Board' }, // r is the leaf now
  ])
...
Deno.test('close on the root clears it — a layout always has a pane', () => {
  ...
  assertEquals(comps(batch), [{ dir: null, content_eid: null, view: null }])
})
...
Deno.test('setContent fills a leaf; swap exchanges two', () => {
  assertEquals(comps(setContent(row, 'c', 'T9', 'Board')), [
    { content_eid: 'T9', view: 'Board' },
  ])
  assertEquals(comps(swap(row, 'a', 'b')), [
    { content_eid: 'T2', view: null },
    { content_eid: 'T1', view: 'Full' },
  ])
})
