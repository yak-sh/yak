...
for (let staged of [false, true]) {
  let path = staged ? 'staged' : 'live'
  let write = (db: Sql, changes: Change[], target?: string, actor?: string) =>
    staged
      ? sync(
        fleetGraphOf(db).apply(
          changes.map((c) => ({
            ...asBundle(c),
            ...(target ? { $workClaim: { target } } : {}),
  ...
  Deno.test(`${path} guards: lease collision rolls back body and audits outside outer tx`, () => {
    let db = bareDb()
    seed(db)
    write(db, [{
      eid: t,
  ...
  Deno.test(`${path} guards: a loser born in a refused batch leaves no phantom session`, () => {
    let db = bareDb()
    seed(db)
    write(db, [{
      eid: t,
  ...
  Deno.test(`${path} guards: named readiness sees approval before claim, never after`, () => {
    let db = bareDb()
    seed(db)
    write(db, [{
      eid: t,
  ...
  Deno.test(`${path} guards: ordered alias members, duplicates, release and rollback`, () => {
    let db = bareDb()
    seed(db)
    write(db, [{
      eid: t,
  ...
  Deno.test(`${path} guards: deleting an alias owner releases primary and members in batch order`, () => {
    let db = bareDb()
    seed(db)
    write(db, [
  ...
  Deno.test(`${path} guards: stop requires managed active, pending graph work or advanceable input`, () => {
    let db = bareDb()
    seed(db)
    let stop = () =>
      write(db, [{
  ...
  Deno.test(`${path} guards: surrounding refusal policy stays atomic`, () => {
    let db = bareDb()
    seed(db)
  ...
  Deno.test(`${path} guards: typed refs see final kinds; missing identities and kind drops refuse`, () => {
    let db = bareDb()
    seed(db)
  ...
  Deno.test(`${path} guards: a new spawn is refused for an undecided proposal, unless decided in its batch`, () => {
    let db = bareDb()
    seed(db)
  ...
  Deno.test(`${path} guards: only the resolved person can accept a proposed memory`, () => {
    let db = bareDb()
    seed(db)
  ...
  Deno.test(`${path} guards: rollback never audits a phantom target or inside an outer transaction`, () => {
    let db = bareDb()
    seed(db)
...
Deno.test('staged $was translates core Stale to human value/hash fields and FOUND semantics', () => {
  let db = bareDb()
  seed(db)
