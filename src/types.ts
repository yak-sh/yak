...
// planted the table, so no store is ever told about another's rows.
let ownKinds = new Set<string>()

export let learnKinds = (names: Iterable<string>) => {
  for (let name of names) if (!kindOrder.includes(name)) ownKinds.add(name)
}
...
export let kindOf = (has: Record<string, unknown>) =>
  [...ownKinds].find((k) => has[k]) ?? kindOrder.find((k) => has[k]) ?? 'entity'
