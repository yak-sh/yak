...
// Precedence as a LOOKUP, keyed by kind. kindOf runs once per row on every
// snapshot read, so it must cost the ENTITY's component count (a handful),
// never the vocabulary's (~50 and growing) — a scan down kindOrder charges
// every row for wherever its kind happens to sit, and kindOrder is derived
// (alphabetical + `before`), so nothing keeps the hot kinds near the front.
let kindRank = new Map(kindOrder.map((k, i) => [k, i]))

// An APP's own components (store/vocab.ts), learned when a store plants them
// (db.ts plantVocab) — the same registry query.ts keeps for the filter
// grammar, safe for the same reason: a row wears a word only if its OWN store
// planted the table, so no store is ever told about another's rows. Ranked
// NEGATIVE in the order learned, so a store's word beats every platform kind
// and an earlier-planted word beats a later one.
let ownKinds = new Map<string, number>()

export let learnKinds = (names: Iterable<string>) => {
  for (let name of names) {
    if (!kindRank.has(name) && !ownKinds.has(name)) {
      ownKinds.set(name, -1 - ownKinds.size)
    }
  }
}
...
export let kindOf = (has: Record<string, unknown>) => {
  ...
  for (let k in has) {
    if (!has[k]) continue
    let r = ownKinds.get(k) ?? kindRank.get(k) ?? Infinity
