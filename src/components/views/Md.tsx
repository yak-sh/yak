...
import { usePersonaProjection } from '../useQuery.ts'
...
// raw is exactly what a spawned session reads. A registered query projection
// computes those bytes from the spawn path's bounded personaGraph closure;
...
export let Md = ({ e }: { e: Ent }) => {
  let persona = usePersonaProjection(e.eid, !!e.persona)
  let text = e.persona ? (persona?.text ?? '…\n') : mdText(e)
