...
// (providers.ts) over the graph's own provider table (catalog.ts) — no view.
// The DOM-mount tests that render <Run/> live in Run_test.ts.
import { assertEquals } from '@std/assert'

Deno.env.set('DB_PATH', ':memory:')
let { db } = await import('../live_db.ts')
let { catalog: table } = await import('../catalog.ts')
let { catalog } = await import('../providers.ts')

// Read the graph's table once at import so the sole test is charged only for
// the catalog logic it is about, not the first traversal.
let ps = table(db)
catalog(ps)

Deno.test('the catalog offers each model once, Sol first, fallback as a transport', () => {
  assertEquals(
    catalog(ps).map((c) => [c.model, c.label, c.transports]),
    [
      ...
      ['gpt-6-astra', 'GPT-6 Astra', ['codex', 'codex-cli']],
