...
slow(
  'commit: takes the paths it wrote, leaves a staged neighbour staged',
  async () => {
    let dir = await repo()
    try {
...
slow(
  'commit: passes over the untracked, the unchanged, and the repo-less',
  async () => {
    let dir = await repo()
    try {
      ...
      Deno.removeSync(loose.slice(0, loose.lastIndexOf('/')), {
        recursive: true,
      })
    } finally {
      Deno.removeSync(dir, { recursive: true })
...
slow(
  'commit: a repo mid-merge keeps the file and reports the refusal',
  async () => {
    let dir = await repo()
    try {
      let file = write(`${dir}/.tasks/AGENTS.md`, 'two\n')
...
slow('standing: level, ahead, behind, and no upstream at all', async () => {
  let w = await paired()
  try {
...
slow('commit: behind-only fast-forwards first, then commits', async () => {
  let w = await paired()
  try {
    await shipped(w)
...
slow(
  'commit: behind on the projection path itself is refused, not forced',
  async () => {
    let w = await paired()
    try {
      ...
      assertEquals(out.failed.length, 1)
      assert(
        out.failed[0].includes('1 behind upstream (0 ahead)'),
        out.failed[0],
      )
...
slow(
  'commit: a diverged branch is refused, the file stays written',
  async () => {
    let w = await paired()
    try {
      await ours(w, 'ours\n')
      ...
      assertEquals(out.failed.length, 1)
      assert(
        out.failed[0].includes('1 behind upstream (1 ahead)'),
        out.failed[0],
      )
...
slow(
  'commit: ahead-only commits, and without the grant nothing leaves',
  async () => {
    let w = await paired()
    try {
      await ours(w, 'ours\n')
...
slow(
  'commit: a granted push drains the whole chain and levels the tree',
  async () => {
    let w = await paired()
    try {
      await ours(w, 'two\n')
...
slow(
  'commit: the grant drains a chain with nothing left to write',
  async () => {
    let w = await paired()
    try {
...
slow('commit: a persistent refusal is reported without retrying', async () => {
  let w = await paired()
  try {
...
slow('commit: a stale push rebases and retries once', async () => {
  let w = await paired()
  try {
...
slow('commit: a second stale push reports its reason and stops', async () => {
  let w = await paired()
  try {
...
slow(
  'commit: the grant does not weaken the refusal to commit onto a behind branch',
  async () => {
    let w = await paired()
    try {
      await shipped(w, '.tasks/AGENTS.md')
