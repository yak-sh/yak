...
//
// The speed bar (./test-budget.ts) is advisory, not a wall (T-17785): it lists
// tests over 1ms slowest-first but exits 0 on the budget alone, turning fatal
// only under TASKS_FAST_STRICT=1.

// Where deno keeps its module cache: DENO_DIR when it is set, which is how
// every session shell on the box pins one cache, otherwise deno's own default
// for the platform.
let denoCache = (() => {
...
// The pinned module cache ({@link denoCache}), with the runner's own
