...
// write brings back, and the other's under ids naming its file. Their logs
// stand at their end, and the files are old and unchanged, so the importer
// reads none of them meanwhile and the server serves on.
//
...
let again = async () => {
  ...
  for (let l of back) {
    ...
    await pull(g, { ...t, line: 0, at: 0 }, claude, {
      prose: true,
      person,
      final: true,
    })
