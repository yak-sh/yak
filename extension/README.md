...
- **It does not normalize the URL.** The address goes over raw both when filing
  ...
  (`packages/page/url.ts`). Two spellings of one page must not be able to
  disagree, and the only way to guarantee that is to have one implementation.
- **It does not parse the `:` line.** Same reason: the server owns that grammar,
  for every door.
