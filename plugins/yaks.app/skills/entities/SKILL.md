...
deploy reports when a component already has a home, what happens across spaces,
how a page reads a sibling app, and how `graph_query` composes one bundle out of
several stores.
...
The platform's shared components — `doc`, `comment`, `task`, `image`, `archived`
...
Within one space a component has one home. Across spaces there is no home to
share, so the same name may describe two different shapes — and the answer tells
you which.
...
A write has no such option — one bundle cannot land in two meanings — so it asks
you:
...
- A component nobody in reach declared is a refusal, not an empty answer:
