...
- **`not_a_writer`** — 401 signed out, with `signIn`; 403 signed in. The app's
  `access` does not accept a write from this person. Signed out, send them to
  `signIn`. Signed in, it is the owner's to grant — the message says so.
- **`not_a_reader`** — 401 signed out, with `signIn`; 403 signed in. A `private`
  app answering someone who is not a member.
- **`not_found`** — 404. No such endpoint under `/api/`. The message lists the
  endpoints that exist: apply, query, me, graph, ws, blob, files/`<path>`.
- **`method_not_allowed`** — 405. The right endpoint, the wrong HTTP method:
  `apply` and `blob` are POST, `blob/<eid>` is GET, `files/<path>` is PUT.
...
Those two lines are the whole of it. `e.message` is the endpoint's `message`, or
...
- **A resource that never loaded** — a script, stylesheet, image, or a module in
  ...
  most: a page paints a heading and empty space and nothing else reports it.
- **An unhandled rejection.** Reported as `unhandled rejection: <the reason>`.
- **A call to one of the app's own `./api/` endpoints that was refused.** It
  wraps `fetch` and watches same-origin responses, passing everything through
  untouched.
...
It makes no other judgement. It sends the response's status and body along with
each refusal, and the platform applies the one rule: a 4xx was somebody's choice
and files nothing.
...
The response headers also name the app's report endpoint as a NEL reporting
...
It also accepts the browser's own Reporting API format — an array of
...
The version number is read fresh rather than from the cache, on purpose. The
...
Sending them marks each one, so the next reply does not repeat them. The mark
...
Ids work there too, so one call can mix them. A break naming no version is
covered by any version bound, and one naming no time by any time bound: nothing
can tell whether those are still happening.
...
Beside the lines it returns the same breaks as data, grouping the ones that
share an app, a message and a place into one card with a count — a render loop
...
keep listed in the message. Pruning past the last 20 deletes only bytes no kept
version names, so the oldest rollback an app still offers always has its files.
...
out, the report is kept and the reply tells you so — no need to send it again.

Three an hour, per person. The fourth is a pause, not a no: the reply says so
and gives the address to write to directly if it cannot wait. Feedback is not
